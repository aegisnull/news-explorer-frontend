...
function Header(props) {
  ...
  const logoPath = (currentPath === '/saved-news') & !menuOpen ? LightLogo : Logo;
  const textColor = currentPath === '/saved-news' ? 'header__link_light' : '';
  const buttonColor =
    (currentPath === '/saved-news') & !menuOpen ? 'header__button_light' : 'header__button';
  const mobileMenu = (currentPath === '/saved-news') & !menuOpen ? MobileMenuDark : MobileMenuLight;
  const logOutIcon = currentPath === '/saved-news' ? 'header__user_light' : 'header__user_dark';
        ...
        {menuOpen ? (
          <>
            <nav className="header__nav header__nav_dropdown">
              <button className="header__mobile-menu" onClick={toggleMenu}>
                <img src={mobileMenu} alt="Mobile Menu" className="header-mobile-menu-img" />
              </button>
            </nav>
            <div className="dropdown">
              <Link to="/">
                <div className={`header__link header__text header__link_dropdown1 `}>Inicio</div>
              </Link>
              {props.isLoggedIn ? (
                <>
                  <Link to="/saved-news">
          ...
          <nav className="header__nav">
            ...
            {props.isLoggedIn ? (
              ...
              <>
                <button className={`${buttonColor} header__text `} onClick={props.onSignInClick}>
                  Iniciar sesión
                </button>
                <button className="header__mobile-menu" onClick={toggleMenu}>
                  <img src={mobileMenu} alt="Mobile Menu" className="header-mobile-menu-img" />
